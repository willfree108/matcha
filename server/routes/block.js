...
.post('/:id', function(req, res) {
  ...
  jwt.verify(token, config.secret, function(err, decoded) {
    ...
    User.block(decoded.id_user, req.params.id, function (err, data) {
      if (err) res.status(500).send()
      else if (data === "same id") res.status(401).send()
      else {
        var user = connected.forEach(s => {
          if (s.idUser === req.params.id) {
            s.emit('notification', `${decoded.username} just blocked you!`)
          }
        })
        User.notif(req.params.id, `${decoded.username} just blocked you!`, function (err, data) {
          if (err) res.status(500).send()
